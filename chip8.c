...
typedef struct {
	...
	int16_t volume;			// Volume of sound
} config_t;
...
bool init_sdl(sdl_t *sdl, config_t *config) {
	...
	// Inititialize audio functionality
...
bool set_config_from_args(config_t *config, int argc, char **argv) {
	...
	*config = (config_t) {
		...
		.insts_per_second = 700,	// Number of instructions to emulate in one second (clock rate of CPU)
...
void print_debug_info(chip8_t *chip8) {
	...
	switch ((chip8->inst.opcode >> 12) & 0x0F) {
		...
		case 0x08:
			switch (chip8->inst.N) {
				...
				case 1:
					...
					printf("Set register V%X (0x%02X) |= V%X (0x%02X); Result: %02X\n",
						chip8->inst.X, chip8->V[chip8->inst.X],
						chip8->inst.Y, chip8->V[chip8->inst.Y],
				...
				case 3:
					...
					break;
				case 4:
					// 0x8XY4: Set register VX += VY; set VF = 1 when there's overflow, 0 when not
					...
					break;
				case 5:
					// 0x8XY5: Set register VX -= VY; set VF = 1 if there's not a borrow (result >= 0)
				...
				case 6:
					...
					printf("Set register V%X (0x%02X) >>= 1, VF = shifted off bit (%X); "
						"Result: 0x%02X\n",
						chip8->inst.X, chip8->V[chip8->inst.X],
					...
					break;
				case 7:
					// 0x8XY7: Set register VX = VY - VX; set VF = 1 if there's not a borrow (result >= 0)
					printf("Set register V%X = V%X (0x%02X) -  V%X (0x%02X), VF = 1 if no borrow; "
						"Result: %02X, VF = %X\n",
						chip8->inst.X, chip8->inst.Y, chip8->V[chip8->inst.Y],
						chip8->inst.X, chip8->V[chip8->inst.X],
						chip8->V[chip8->inst.Y] - chip8->V[chip8->inst.X],
					...
					break;
				case 0xE:
					// 0x8XYE: Set the register VX <<= 1, store shifted off bit in VF
					printf("Set register V%X (0x%02X) <<= 1, VF = shifted off bit (%X); "
						"Result: 0x%02X\n",
						chip8->inst.X, chip8->V[chip8->inst.X],
		...
		case 0x0B:
			...
			break;
		case 0x0C:
			// 0xCXNN: Set VX to the result of NN & a random number (0-255)
		...
		case 0x0F:
			switch (chip8->inst.NN) {
				case 0x0A:
					...
					printf("Await a key press; store key in V%X\n", chip8->inst.X);
					break;
				...
				case 0x29:
					...
					printf("Set I to sprite location in memory for character in V%X (0x%02X); "
						"Result (VX*5) = (0x%02X)\n",
				...
				case 0x65:
					// 0xFX565: Register load V0-VX inclusive from memory offset from I;
					//	SCHIP does not increment I, CHIP8 does increment I
	...
	switch ((chip8->inst.opcode >> 12) & 0x0F) {
		...
		case 0x03:
			...
			if (chip8->V[chip8->inst.X] == chip8->inst.NN)
				chip8->PC += 2; // Skip the next opcode/instruction
			break;
		...
		case 0x0F:
			switch (chip8->inst.NN) {
				case 0x0A:
					...
					// If no key has been pressed yet, keep getting the current opcode and running this instruction
				...
				case 0x29:
					// 0xFX29: Set register I to sprite location in memory for character VX (0x0 - 0xF)
				...
				case 0x55:
					// 0xFX55: Register dump V0-VX inclusive to memory offset from I;
					//	SCHIP does not increment I, CHIP8 does increment I
					//	NOTE: Could make this a config flag to use SCHIP or CHIP8 logic
...
int main(int argc, char **argv) {
	...
	while (chip8.state != QUIT) {
		...
		// Delay for approximately 60hz/60fps (16.67 ms) or actual time elapsed
